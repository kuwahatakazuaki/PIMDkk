module python_mace_interface
  use iso_c_binding
  implicit none
  private
  public :: mace_initialize
  public :: mace_finalize
  public :: mace_calculate_energy_and_forces
  ...
  character(len=*), parameter :: module_name = 'mace_function'
...
contains
  ...
  subroutine mace_initialize(extra_sys_path)
    implicit none
    ...
    python_ready = .true.
  end subroutine mace_initialize

  subroutine mace_finalize()
    implicit none
  ...
  subroutine mace_calculate_energy_and_forces( &
    ...
    if (.not. python_ready) then
      if (present(extra_sys_path)) then
        call mace_initialize(extra_sys_path)
      else
        call mace_initialize()
      end if
    end if
    ...
    py_model_path = PyUnicode_FromString(to_c_chars(model_path))
    call ensure_ptr(py_model_path, 'convert model path')
    ...
    call ensure_ok( &
         PyTuple_SetItem(args_ptr, 2_c_ptrdiff_t, py_model_path), &
         'set model_path argument')
    call ensure_ok( &
...
end module python_mace_interface
